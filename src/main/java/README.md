...
### 环境
- CPU: Intel i9-12900, 14核20线程
- 内存: 32GB（Flink限制68M测试）
- Flink: 1.18, HashMapStateBackend
- 并行度: 4

### 测试场景
| 场景 | 配置 | 瓶颈类型 |
|:---|:---|:---|
| 场景A：均匀分布 | push 33%/sms 33%/email 33% | 正常功能验证 |
| 场景B：数据倾斜 | push 75%/sms 12.5%/email 12.5% | CPU瓶颈 |
| 场景C：双重瓶颈 | push 75% + 内存限制68M | **CPU+内存双重瓶颈** |
...
| 0分钟 | CPU正常，Web UI绿色 |
| 3分钟 | 单核CPU飙高（数据倾斜） |
| 5分钟 | Web UI Busy 100%（红色） |
| 16:48 | 卡顿出现（CPU瓶颈） |
| 16:57 | **内存改68M，Web UI不可看，卡顿加剧** |

### 单一瓶颈 vs 双重瓶颈
| 维度 | CPU瓶颈（场景B） | 双重瓶颈（场景C） |
|:---|:---|:---|
| Web UI | Busy 100%，可看 | **不可看，服务挂掉** |
| 输出 | 卡顿 | **严重卡顿** |
| OOM | 未出现 | **接近崩溃** |
| 解决优先级 | 数据倾斜 | **先内存保命，再优化倾斜** |
...
- 分支1（成功率）：✅ 正常
- 分支2（连续异常）：✅ 正常
- 分支3（延迟监控）：✅ 正常（延迟数据在运行一段时间后出现）

### 生产环境解决
1. **CPU瓶颈**：加盐`keyBy(channel + salt)` / 调并行度 / 缩小窗口
2. **内存瓶颈**：RocksDB状态后端 / 调大TaskManager内存
3. **双重瓶颈**：先RocksDB保状态不丢，再加盐打散key，最后调并行度

### 与项目1对比
| 项目 | 瓶颈类型 | 临界点 |
|:---|:---|:---|
| 项目1：实时质量看板 | 内存OOM | 69MB/68MB精确临界点 |
| 项目2：数据质量监控 | CPU+内存双重瓶颈 | Busy 100% + Web UI挂掉 |

### 可靠性验证：Checkpoint
| 时间 | 现象                          |
|:---|:----------------------------|
| 17:06 | Checkpoint正常生成，10秒间隔，文件落盘正常 |
