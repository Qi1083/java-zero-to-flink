...
import com.qi.realtime.monitor.source.DataEventSource;
import com.qi.realtime.monitor.utils.TimeUtil;
...
public class DataQualityMonitor {
    public static void main(String[] args) throws Exception {
        ...
        SingleOutputStreamOperator<com.qi.realtime.monitor.entity.DataEventEntity> eventSource = env.addSource(new DataEventSource())
        ...
        SingleOutputStreamOperator<String> windowEvent = eventSource.keyBy(com.qi.realtime.monitor.entity.DataEventEntity::getChannel)
                ...
                .aggregate(
                        new AggregateFunction<DataEventEntity, Tuple2<Integer, Integer>, Tuple2<Integer, Integer>>() {
                            ...
                            @Override
                            public Tuple2<Integer, Integer> add(com.qi.realtime.monitor.entity.DataEventEntity event, Tuple2<Integer, Integer> accumulator) {
        ...
        eventSource.keyBy(com.qi.realtime.monitor.entity.DataEventEntity::getSouceId)
    ...
    public static class ConsecutiveFailAlertFunction extends KeyedProcessFunction<String, DataEventEntity, com.qi.realtime.monitor.entity.DataEventEntity> {
        ...
        @Override
        public void processElement(com.qi.realtime.monitor.entity.DataEventEntity event, KeyedProcessFunction<String, com.qi.realtime.monitor.entity.DataEventEntity, com.qi.realtime.monitor.entity.DataEventEntity>.Context ctx, Collector<com.qi.realtime.monitor.entity.DataEventEntity> out) throws Exception {
            ...
            if ("FAIL".equalsIgnoreCase(event.getStatus())) {
                ...
                if ((count >= 3) && (now - time > Thread)) {
                    ctx.output(ALERT_TAG, new com.qi.realtime.monitor.entity.DataEventEntity(
                            event.getSouceId(),
                            event.getStatus(),
                            now,
                            event.getChannel()
